...
# Use any speaker IP
speaker = soco.SoCo("192.168.4.36")

# Always use the group coordinator
try:
    coordinator = speaker.group.coordinator
    log(f"Selected speaker: {speaker.player_name}")
    log(f"Using coordinator: {coordinator.player_name}")
except Exception as e:
    log(f"Error determining coordinator: {e}")
    coordinator = speaker  # fallback
